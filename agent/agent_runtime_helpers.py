    ...
    if effective_reason == FailoverReason.auth:
        ...
        if (
            ...
            _is_xai_auth_failure = (
                ...
                or "oauth2 access token could not be validated"
                in _disambiguator_haystack
            )
    ...
    if function_name == "session_search":
        ...
        def _execute(next_args: dict) -> Any:
            ...
            if entry and entry.handler:
                return _finish_agent_tool(entry.handler(next_args), next_args)
            return _finish_agent_tool(
                json.dumps({"error": "todo_list tool not found in registry"}), next_args
            )
