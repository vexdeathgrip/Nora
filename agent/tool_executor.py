...
def _log_nora_retry(tool_name: str, tool_calls: list) -> None:
    """Log a _nora_retry signal for the nora-memory.log file."""
    _nora_logger = logging.getLogger("nora_memory")
    call_names = [tc.get("function", {}).get("name", "?") for tc in (tool_calls or [])]
    _nora_logger.warning(
        "[RETRY] %s — assistant content cleared. tool_calls=%s", tool_name, call_names,
    )
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, (tc, name, args, middleware_trace, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
        ...
        # ── Nora retry signal cleanup ──────────────────────────────
        # When a tool signals _nora_retry, clear the assistant message content
        # so the failed analysis text is removed from context. This prevents
        # loop patterns and forces a prompt cache miss. We keep the message
        # structure (tool_calls intact) so message role alternation is valid.
        if isinstance(function_result, str):
            try:
                _retry_data = json.loads(function_result)
                if isinstance(_retry_data, dict) and _retry_data.get("_nora_retry"):
                    for i in range(len(messages) - 1, -1, -1):
                        m = messages[i]
                        if isinstance(m, dict) and m.get("role") == "assistant" and m.get("tool_calls"):
                            if isinstance(m.get("content"), str) and len(m.get("content", "")) > 0:
                                m["content"] = None
                                _log_nora_retry(name, m.get("tool_calls", []))
                            break
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _block_msg is not None:
            ...
            try:
                def _execute(next_args: dict) -> Any:
                    ...
                    entry = registry.get_entry("todo_list")
                    if entry and entry.handler:
                        return entry.handler(next_args)
                    return json.dumps({"error": "todo_list tool not found in registry"})
                function_result, function_args = _run_agent_tool_execution_middleware(
                    agent,
                    function_name=function_name,
                    function_args=function_args,
                ...
                _todo_result = function_result
            except Exception as tool_error:
                function_result = json.dumps({"error": f"todo_list failed: {tool_error}"})
                logger.error("todo_list raised for %s: %s", function_name, tool_error, exc_info=True)
            finally:
                tool_duration = time.time() - tool_start_time
                cute_msg = _get_cute_tool_message_impl(function_name, function_args, tool_duration, result=_todo_result)
                if spinner:
                    spinner.stop(cute_msg)
                elif agent._should_emit_quiet_tool_messages():
                    agent._vprint(f"  {cute_msg}")
        ...
        # ── Nora retry signal cleanup (sequential) ──────────────────
        if isinstance(function_result, str):
            try:
                _retry_data = json.loads(function_result)
                if isinstance(_retry_data, dict) and _retry_data.get("_nora_retry"):
                    for _ri in range(len(messages) - 1, -1, -1):
                        _rm = messages[_ri]
                        if isinstance(_rm, dict) and _rm.get("role") == "assistant" and _rm.get("tool_calls"):
                            if isinstance(_rm.get("content"), str) and len(_rm.get("content", "")) > 0:
                                _rm["content"] = None
                                _log_nora_retry(function_name, _rm.get("tool_calls", []))
                            break
            except (json.JSONDecodeError, TypeError):
                pass
        ...
        # injection lands as soon as a tool finishes — not against the
