...
def _import_skill_store():
    ...
    skill_store_path = get_hermes_home() / "plugins" / "vector-context" / "skill_store.py"
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    # Index the new skill into the vector store
    try:
        skill_store = _import_skill_store()
        if skill_store:
            skill_store.index_skill(name, str(skill_md), "active")
    except Exception:
        pass  # Indexing failure should not block skill creation
...
def _edit_skill(name: str, content: str) -> Dict[str, Any]:
    ...
    # Re-index the updated skill into the vector store
    try:
        skill_store = _import_skill_store()
        if skill_store:
            skill_store.index_skill(name, str(skill_md), "active")
    except Exception:
        pass  # Indexing failure should not block skill update
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        skill_store = _import_skill_store()
        if skill_store:
            skill_store.remove_skill(name)
    except Exception:
        pass  # Indexing failure should not block skill deletion
