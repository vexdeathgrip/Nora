...
# Rate limiting for SearXNG-backed web search tools
# SearXNG's upstream engines (Brave, Startpage) rate-limit rapid requests.
# Track last call time per tool and add randomized delay between calls.
_web_search_last_call: float = 0.0
_web_search_lock = threading.Lock()
_WEB_SEARCH_MIN_DELAY = 1.0  # minimum seconds between web search calls
_WEB_SEARCH_MAX_DELAY = 5.0  # maximum seconds between web search calls
_WEB_SEARCH_TOOLS = frozenset({"web_search", "mcp_universal_web_search"})
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        # Initialize web search timestamp to avoid UnboundLocalError on
        # first call (variable assigned at line 2663, read at line 2652).
        _web_search_last_call = 0.0
        ...
        # Rate-limit web search tools to avoid SearXNG upstream engine throttling.
        # Add randomized delay (1-5s) between consecutive web search calls.
        if tool_name in _WEB_SEARCH_TOOLS:
            import random
            with _web_search_lock:
                now = time.monotonic()
                elapsed = now - _web_search_last_call
                if elapsed < _WEB_SEARCH_MIN_DELAY:
                    delay = random.uniform(_WEB_SEARCH_MIN_DELAY, _WEB_SEARCH_MAX_DELAY)
                    # Don't wait the full delay if we've already waited some
                    delay = max(0, delay - elapsed)
                    if delay > 0:
                        logger.debug(
                            "Rate-limiting %s: waiting %.1fs (last call %.1fs ago)",
                            tool_name, delay, elapsed,
                        )
                        time.sleep(delay)
                _web_search_last_call = time.monotonic()
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        is_connecting = server is not None and server.session is None and server._error is None
        if server and server.session is not None:
            ...
            # A server in _servers with session=None but no error is still
            # connecting (background MCP discovery hasn't finished yet).
            result.append({
                "name": name,
                "transport": transport,
                "tools": 0,
...
def discover_mcp_tools_list() -> str:
    """List all available MCP tools across all servers.
    
    Returns a JSON string with tool names, descriptions, and servers.
    """
    import json
            ...
            if name.startswith("mcp_"):
                ...
                if len(parts) >= 3:
                    ...
                    tools_by_server[server].append({
                        "name": tool_name,
                        "full_name": name,
                        "description": (entry.schema.get("description", "")[:100] if entry else ""),
                    })
    except Exception as e:
        return json.dumps({"error": str(e)})
    
    if not tools_by_server:
        return json.dumps({"message": "No MCP tools available"})
...
def _discover_tools_handler(args: dict, **kw) -> str:
    """Three-step interactive MCP tool selection.

    Step 1 (list):  discover_tools() or discover_tools(action="list")
    Step 2 (select): discover_tools(tool_name="<name>")
    Step 3 (call):   Call the tool directly now that it is registered
    Step 4 (quit):    discover_tools(action="quit")
    """
    import json
    import re
    ...
    # Validate tool_name contains only safe characters
    if tool_name and not re.match(r'^[a-zA-Z0-9_\-]+$', tool_name):
        return json.dumps({
            "error": f"Invalid tool name: '{tool_name}'.",
            "instruction": "Tool names contain only letters, numbers, underscores, and hyphens.",
        })

    from tools.registry import registry

    # Step 3: Quit (check before list so quit works even without MCP tools)
    if action == "quit":
        try:
            from tools._mcp_bridge import clear_pending
            clear_pending()
        except ImportError:
            pass
        return json.dumps({
            "result": "cancelled",
            "message": "Tool selection cancelled. You can call discover_tools() again later.",
        })

    # Step 1: List all available MCP tools
    if not tool_name or action == "list":
        available = _get_mcp_tool_names(registry)
        if not available:
            return json.dumps({"result": "none", "message": "No MCP tools available."})
        return json.dumps({
            "result": "list",
            "available_tools": available,
            "instruction": (
                "Pick one tool from 'available_tools' above. "
                "Call discover_tools(tool_name=\"<name>\") to see its schema and make it available."
            ),
            "hint": "Call discover_tools(action=\"quit\") to cancel selection.",
        }, indent=2)

    # Step 2: Select a specific tool
    # Build mapping: user-friendly name -> full MCP tool name
    name_map = {}
    for name in registry.get_all_tool_names():
        if name.startswith("mcp_"):
            ...
            if len(parts) >= 3:
                server = parts[1]
                simple = "_".join(parts[2:])
                name_map[simple] = name
                name_map[simple.lower()] = name
                name_map[name] = name  # also accept full MCP name
                if server:
                    name_map[f"{server}_{simple}"] = name

    full_name = name_map.get(tool_name) or name_map.get(tool_name.lower())
    if not full_name:
        available = _get_mcp_tool_names(registry)
        return json.dumps({
            "error": f"Unknown tool '{tool_name}'.",
            "available_tools": available,
            "instruction": "Pick a tool from 'available_tools' or call discover_tools(action=\"quit\") to cancel.",
        }, indent=2)

    entry = registry.get_entry(full_name)
    if not entry:
        return json.dumps({"error": f"Internal error: '{full_name}' not in registry."})
    ...
    try:
        from tools._mcp_bridge import set_pending_tool
        set_pending_tool(full_name, schema)
    except ImportError:
        pass
...
def _get_mcp_tool_names(registry) -> list:
    """Return a clean list of available MCP tool names (user-friendly)."""
    tools = []
    for name in registry.get_all_tool_names():
        if name.startswith("mcp_"):
            parts = name.split("_", 2)
            if len(parts) >= 3:
                simple = "_".join(parts[2:])
                server = parts[1]
                entry = registry.get_schema(name)
                desc = ""
                if entry:
...
DISCOVER_TOOLS_SCHEMA = {
    "name": "discover_tools",
    "description": (
        "Interactive MCP tool discovery. Step 1: list all available MCP tools. "
        "Step 2: select a tool to see its schema. Step 3: call it directly."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "enum": ["list", "quit"],
                "description": "Action: list (default) or quit",
            },
            "tool_name": {
                "type": "string",
                "description": "Name of tool to inspect and enable",
            },
        },
    },
}
