...
def install_from_quarantine(
    ...
    # Index the hub-installed skill into the vector store
    try:
        skill_store = _import_skill_store()
        if skill_store:
            skill_md = install_dir / "SKILL.md"
            if skill_md.exists():
                skill_store.index_skill(safe_skill_name, str(skill_md), "hub")
    except Exception:
        pass  # Indexing failure should not block skill install
...
def uninstall_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    try:
        skill_store = _import_skill_store()
        if skill_store:
            skill_store.remove_skill(skill_name)
    except Exception:
        pass  # Indexing failure should not block skill uninstall
